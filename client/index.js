...
function tcontroller($scope, $http) {
  ...
  $scope.getPlayer = function (id) {
    $http.get(`/api/players/id/${id}`).then((player) => {
  ...
  $scope.getTeams = function () {
    $http
      .get("/api/teams?filter=%7B%22include%22%3A%20%22players%22%7D")
      .then((team) => {
  ...
  $scope.Sold = function () {
    const playerUpdate = new Promise((resolve, reject) => {
      $http
        .patch(`/api/players/${$scope.player.id}`, {
          status: "Sold",
    ...
    const teamUpdate = new Promise((resolve, reject) => {
      $http
        .patch(`/api/teams/${$scope.selectedTeam.id}`, {
    ...
    Promise.all([playerUpdate, teamUpdate]).then(() => {
      $scope.selectedTeam = "";
      $scope.currentBid = null;
      $scope.getTeams();
      $scope.getPlayer($scope.nextPlayerId);
  ...
  $scope.getUnsold = function () {
    $http
      .get(
        `/api/players?filter=%7B%22where%22%3A%7B%22status%22%3A%20%22UnSold%22%7D%7D`
      )
  ...
  $scope.getCarousel = function () {
    $http
      .get(
        "/api/players?filter=%7B%22where%22%3A%7B%20%22or%22%3A%20%5B%20%7B%22status%22%3A%20%22Sold%22%7D%2C%20%7B%22status%22%3A%20%22UnSold%22%7D%5D%7D%2C%20%22include%22%20%3A%20%22teams%22%7D"
      )
      .then((data) => {
